...
[![crates.io shield](https://img.shields.io/crates/v/agentmail_sdk)](https://crates.io/crates/agentmail_sdk)
...
```rust
match client.inboxes.create(None)?.await {
    Ok(response) => {
...
let request = PodsCreatePodRequest {
    ...
};
```
...
```rust
let response = client.inboxes.create(
